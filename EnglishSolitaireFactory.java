...
/**
 * Represents a factory for creating instances of the English Solitaire game.
 */
public class EnglishSolitaireFactory implements MarbleSolitaireAbstractFactory{
	...
	/**
	 * Constructs an EnglishSolitaireFactory instance with the default arm size of 3.
	 */
	public EnglishSolitaireFactory() {
	}

	/**
	 * Constructs an EnglishSolitaireFactory instance with the specified arm size.
	 *
	 * @param arm the arm size of the game board
	 */
	public EnglishSolitaireFactory(int arm) {
		this.arm = arm;

	}

	/**
	 * Constructs an EnglishSolitaireFactory instance with the specified arm size and empty slot position.
	 *
	 * @param arm the arm size of the game board
	 * @param x   the x-coordinate of the empty slot position
	 * @param y   the y-coordinate of the empty slot position
	 */
	public EnglishSolitaireFactory(int arm, int x, int y) {
		this(arm);
		this.x = x;
		this.y = y;
	...
	/**
	 * Creates a new instance of the EnglishSolitaireModel.
	 *
	 * @return a new instance of the EnglishSolitaireModel
	 */
	@Override
	public MarbleSolitaireModel createModel() {

		if ( x == -1 || y == -1 ) {
			return new EnglishSolitaireModel(arm);
		}

		return new EnglishSolitaireModel(arm, x, y);
